...
class _rsvpPageState extends State<rsvpPage> {
  ...
  void _fetchEventData() {
    FirebaseFirestore.instance
        ...
        .doc('mgoyBWf4Gr5vhg0yM7qC')
        .snapshots()
        .listen((snapshot) {
      if (snapshot.exists) {
        Timestamp eventTimestamp = snapshot['time']; // Timestamp field in Firestore
        ...
        setState(() {
          ...
          isTimeEnded = remainingTime <= Duration.zero; // Check time immediately
        });
  ...
  Future<void> _handleRSVP() async {
    ...
    if (user != null) {
      String userEmail = user.email ?? 'Unknown'; // Default to 'Unknown' if no email exists
      DateTime timestamp = DateTime.now(); // Current timestamp

      // Reference to the Firestore collection
      CollectionReference rsvpCollection = FirebaseFirestore.instance.collection('rsvp');

      // Check if an RSVP already exists for the user
      QuerySnapshot querySnapshot = await rsvpCollection.where('teamID', isEqualTo: userEmail).get();
                ...
                isLoading
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        Text(
                          isTimerStarted ? 'Event Starts In :' : 'Loading...',
